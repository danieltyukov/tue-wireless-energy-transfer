...
float meas_2_old = 0;
float meas_3_old = 0;
// meas_1 -> input Voltage DC-DC
// meas_2 -> current of DC-DC converter (entire circuit)
// meas_3 -> output Voltage DC-DC
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_TIM3_Init();
  /* USER CODE BEGIN 2 */
 // set base dutycycle and frequencies of signals
 // start PWM and basetime
 // 2 complementary PWM signals for DC-AC
 // 1 PWM signal for DC-DC
  ...
  while (1)
  {
	  ...
	  // checks deltas of output Voltage and current as well as incremental changes DI/DV
	  	...
	  	if (deltaV == 0){
	  		...
	  		if (deltaI>0){
	  			PWM_DutyC_DCDC = PWM_DutyC_DCDC + 0.5;
	  	...
	  	if ((meas_2 + (deltaI)/(deltaV)*meas_3) == 0){
...
raw_meas_1 = adc_buff[0];
raw_meas_2 = adc_buff[1];
raw_meas_3 = adc_buff[2];
meas_1 = (((float)raw_meas_1) / 4095.0f) *3.3f*20; //voltage in DCDC
meas_2 = (((float)raw_meas_2) / 4095.0f) *3.3f*20*1000; //current DCDC
meas_3 = (((float)raw_meas_3) / 4095.0f) *3.3f*20; // voltage out DCDC
}
/* USER CODE END 4 */
